...
// Загружаем CSV и строим график
fetch('data.csv')
  .then(r => r.text())
  .then(text => {
    const parsed = Papa.parse(text, { header: true, dynamicTyping: true });
    const rows = parsed.data.filter(r => r.Year);
    ...
    const age18_24 = rows.map(r => r['18 to 24 years']);
    const age25_44 = rows.map(r => r['25 to 44 years']);
    const age45_64 = rows.map(r => r['45 to 64 years']);
    const age65 = rows.map(r => r['65 years and older']);
